<template>
  <div>
    <v-flex d-flex>
      <v-layout wrap class="mx-10">
        <v-flex v-for="personaje in personajes" :key="personaje.id" md4>
          <v-card class="card-container mt-8 mx-1 pb-2" max-width="500px">
            <v-img :src="personaje.imagen" height="100px width:400px" class="ma-auto" />

            <v-card-title class="pt-3">{{ personaje.nombre }} </v-card-title>
            </v-card>
          </v-flex>
    </v-layout>
    </v-flex>
